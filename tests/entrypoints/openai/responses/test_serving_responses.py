    ...
    async def test_render_response_inputs_reuses_generation_renderer(
        ...
        serving_responses_instance.online_renderer.render_responses = AsyncMock(
            return_value=SimpleNamespace(
                ...
                engine_input=tokens_input([3, 4, 5]),
            )
        )
        request = ResponsesRequest(
            model="test-model",
        ...
        serving_responses_instance.online_renderer.render_responses.assert_awaited_once_with(
            request,
            previous_messages=None,
            previous_response_outputs=None,
            tool_server=serving_responses_instance.tool_server,
            skip_mm_cache=True,
        )
        serving_responses_instance.engine_client.generate.assert_not_called()

    @pytest.mark.asyncio
    async def test_render_response_inputs_passes_explicit_state_without_mutating_store(
        self, serving_responses_instance
    ):
        serving_responses_instance._check_model = AsyncMock(return_value=None)
        ...
        previous_response = SimpleNamespace(
            id="resp_1",
            output=previous_outputs,
        )
        serving_responses_instance.response_store["resp_1"] = previous_response
        serving_responses_instance.msg_store["resp_1"] = previous_messages
        serving_responses_instance.online_renderer.render_responses = AsyncMock(
            return_value=SimpleNamespace(
                messages=[*previous_messages, {"role": "user", "content": "next"}],
                engine_input=tokens_input([6, 7]),
            )
        )
        request = ResponsesRequest(
            model="test-model",
        ...
        call_kwargs = (
            serving_responses_instance.online_renderer.render_responses.call_args.kwargs
        )
        assert call_kwargs["previous_messages"] == previous_messages
        assert call_kwargs["previous_messages"] is not previous_messages
        assert call_kwargs["previous_response_outputs"] == previous_outputs
        assert call_kwargs["previous_response_outputs"] is not previous_outputs
        assert serving_responses_instance.msg_store["resp_1"] == previous_messages
        assert previous_response.output == previous_outputs
        serving_responses_instance.engine_client.generate.assert_not_called()
